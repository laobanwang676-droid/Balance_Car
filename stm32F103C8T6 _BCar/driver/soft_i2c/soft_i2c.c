...
void Soft_I2C_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);//开启AFIO时钟,以便复用功能IO口
    GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//关闭JTAG复用功能，保留SWD功能，使PB3、PB4可作为普通IO口使用
    //否则软i2c无法使用PB3、PB4引脚
    // 2. 配置引脚
